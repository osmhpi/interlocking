...
,
    ...
    pub fn IsRouteSet(&self, ctx: &EvalContext, now: timestamp) -> bool {
        (match (match (match (match (Some(self.entity.element_right_port_a.iter().any(|name| ctx.RouteFindingSearch.get(name).unwrap().Check == RouteChecking::ROUTE_SET_FROM_B)), Some(self.entity.element_right_port_a.iter().any(|name| ctx.RouteFindingSearch.get(name).unwrap().Check == RouteChecking::ROUTE_SET_FROM_C))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_b.iter().any(|name| ctx.RouteFindingSearch.get(name).unwrap().Check == RouteChecking::ROUTE_SET_FROM_A))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_c.iter().any(|name| ctx.RouteFindingSearch.get(name).unwrap().Check == RouteChecking::ROUTE_SET_FROM_A))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_b.iter().any(|name| ctx.RouteFindingSearch.get(name).unwrap().Check == RouteChecking::ROUTE_SET_FROM_START))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }).unwrap_or(false)
    }
    ...
    pub fn IsRouteSetDownstream(&self, ctx: &EvalContext, now: timestamp) -> bool {
        (match (match (match (match (Some(self.entity.element_right_port_a.iter().any(|name| ctx.RouteFindingResponse.get(name).unwrap().Admit == RouteAdmission::ROUTE_TO_B)), Some(self.entity.element_right_port_a.iter().any(|name| ctx.RouteFindingResponse.get(name).unwrap().Admit == RouteAdmission::ROUTE_TO_C))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_a.iter().any(|name| ctx.RouteFindingResponse.get(name).unwrap().Admit == RouteAdmission::ROUTE_END))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_b.iter().any(|name| ctx.RouteFindingResponse.get(name).unwrap().Admit == RouteAdmission::ROUTE_TO_A))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }, Some(self.entity.element_right_port_c.iter().any(|name| ctx.RouteFindingResponse.get(name).unwrap().Admit == RouteAdmission::ROUTE_TO_A))) {
    (Some(a), Some(b)) => Some(a || b),
    _ => None
  }).unwrap_or(false)
    }
