    ...
    // Auto-generated Rust state machine for SignalClaim
...
pub struct SignalClaimStateMachine {
    __state: root_State,
...
impl Graph for SignalClaimStateMachine {
...
impl SignalClaimStateMachine {
    ...
    fn transition_from_root___initial(&mut self, now: timestamp) -> root_State {
                    ...
                    web_sys::console::log_1(&format!("SignalClaim({})=SIGNAL_NOT_START", self.entity.name).into());
    ...
    fn transition_from_root_SIGNAL_NOT_START(&mut self, now: timestamp) -> root_State {
        if self.RouteIsSet_value {
            self.State = ActiveInactive::ACTIVE;
                        web_sys::console::log_1(&format!("SignalClaim({})=SIGNAL_IS_START", self.entity.name).into());
