    ...
    // Auto-generated Rust state machine for PointLockLeft
...
impl Graph for PointLockLeftStateMachine {
    ...
    fn transition_from_root___initial(&mut self, now: timestamp) -> root_State {
        if self.PointInOppositePosition_value {
            self.State = ActiveInactive::INACTIVE;
                        web_sys::console::log_1(&format!("PointLockLeft({})=NOT_LOCKED", self.entity.name).into());
            return root_State::NOT_LOCKED; }
                    self.State = ActiveInactive::ACTIVE;
                    web_sys::console::log_1(&format!("PointLockLeft({})=LOCKED", self.entity.name).into());
